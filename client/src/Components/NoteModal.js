...
function NoteModal({ mode, setOpenModal }) {
  ...
  return (
    <div className='h-screen w-screen fixed flex justify-center items-center z-10 inset-0 bg-chalkboardLight bg-opacity-50'>
      <div className='flex flex-col h-max-3/4 w-2/5 min-w-80 rounded-lg bg-chalkboard'>
        ...
        <form className='flex flex-col justify-evenly p-4 grow'>
          <div className='flex flex-col mb-4'>
            <label htmlFor='title' className='text-white'>Title</label>
            <input type='text' name='title' required className='rounded-md p-2 mt-2' />
          </div>
          <div className='flex flex-col mb-4'>
            <label htmlFor='description' className='text-white'>Description</label>
            <textarea name='description' className='min-h-20 max-h-40 rounded-md p-2 mt-2' />
          </div>
          <div className='flex flex-col'>
            <label htmlFor='date' className='text-white'>Due Date</label>
            <input type='date' min={currentDate} className='rounded-md p-2 mt-2' />
          </div>
        </form>
        <div className='flex justify-between  border-t border-black p-4'>
          ...
          {isEdit ?
            <button className='text-white bg-yellow-900 rounded-md px-4 py-1 hover:bg-yellow-700'>Save Changes</button> :
            <button className='text-white bg-yellow-900 rounded-md px-4 py-1 hover:bg-yellow-700'>Create Note</button>}
        </div>
      </div>
    </div>
  )
